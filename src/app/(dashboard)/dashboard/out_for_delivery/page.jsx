...
export default function OutForDeliveryPage() {
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    const toastId = toast.loading(`Updating order #${paymentModalOrder.order_id}...`)
  ...
  const handleCancelOrder = async (orderId) => {
    if (!window.confirm(`Are you sure you want to cancel order #${orderId}? Items will be restocked and no payment recorded.`)) return
    const toastId = toast.loading(`Cancelling order #${orderId}...`)
    try {
      await axios.put(`/api/sale/${orderId}`, { status: 'cancelled' })
      toast.success(`Order #${orderId} cancelled & restocked`, { id: toastId })
      fetchOutForDeliveryOrders()
    } catch (err) {
    ...
    <div className={`w-full min-h-screen bg-slate-50 pt-20 pb-12 px-2 sm:px-4 md:px-8 transition-all duration-300 ${dashSidebar ? 'lg:pl-68' : 'lg:pl-8'}`}>
      <div className="w-full flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 pb-4">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-slate-800 tracking-tight">Out for Delivery Desk</h1>
            <p className="text-xs text-slate-500 mt-0.5">Manage orders currently out for delivery. Mark as delivered on receipt, or process returns.</p>
          </div>
          <button
        ...
        {loading ? (
          <div className="w-full py-20 flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-slate-500 text-xs font-semibold animate-pulse">Fetching out for delivery orders...</p>
          </div>
        ) : orders.length === 0 ? (
                  ...
                  return (
                    <tr key={order.order_id} className="hover:bg-slate-50/70 transition">
                      ...
                      <td className="px-2 sm:px-3 py-3.5 text-center">
                        <span className="px-1.5 py-0.5 text-[9px] sm:text-[10px] font-bold uppercase border border-sky-200 bg-sky-50 text-sky-700">
                          {order.status}
                        </span>
                      </td>
                      <td className="px-2 sm:px-3 py-3.5 text-center relative action-menu-container">
                        ...
                        {isMenuOpen && (
                          <div className="absolute right-2 top-11 w-44 bg-white border border-slate-200 shadow-lg z-30 flex flex-col divide-y divide-slate-100 py-1 text-left">
                            ...
                            >
                              <BiCheckCircle className="text-emerald-600 text-base" /> Mark Delivered
                            </button>
