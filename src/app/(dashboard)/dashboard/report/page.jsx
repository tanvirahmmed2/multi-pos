  ...
  const handlePrintReport = () => {
    ...
    const trendRows = reportData.salesTrend.map(row => `
      <tr>
        ...
        <td style="padding: 8px 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600;">αº│${parseFloat(row.total).toFixed(2)}</td>
      </tr>
    `).join('')
    ...
    const productRows = reportData.topProducts.map((row, idx) => `
      <tr>
        ...
        <td style="padding: 8px 12px; border-bottom: 1px solid #e2e8f0; text-align: right; font-weight: 600;">αº│${parseFloat(row.revenue).toFixed(2)}</td>
      </tr>
    `).join('')
      ...
      <html>
        ...
        <body>
          ...
          <div style="margin-top: 60px; text-align: center; font-size: 10px; color: #94a3b8; border-top: 1px dashed #e2e8f0; padding-top: 16px;">
            Confidential Report ΓÇö Internal Management Use Only ΓÇö Computer Generated Receipt
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-5">
          <div className="bg-white border border-slate-200 p-5 shadow-sm flex items-center justify-between">
            <div>
              ...
              <h3 className="text-base md:text-lg font-bold text-slate-800 mt-0.5">αº│{totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2 })}</h3>
            </div>
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {reportData.topProducts.map((row, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 transition">
                        ...
                        <td className="px-2.5 py-2 text-right font-bold text-emerald-600">αº│{parseFloat(row.revenue).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {reportData.salesTrend.map((row, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 transition">
                        ...
                        <td className="px-2.5 py-2 text-right font-bold text-emerald-600">αº│{parseFloat(row.total).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                {reportData.paymentBreakdown.map((row, idx) => (
                  <div key={idx} className="bg-slate-50 border border-slate-200 p-4 flex justify-between items-center">
                    ...
                    <div className="text-right">
                      ...
                      <h4 className="text-sm font-bold text-emerald-600 mt-0.5">αº│{parseFloat(row.total).toFixed(2)}</h4>
                    </div>
                  </div>
                ))}
              </div>
