...
export default function DashboardExpensePaymentsPage() {
  ...
  const fetchPayments = async () => {
    try {
      const res = await axios.get('/api/expense-payments')
      setPayments(res.data)
    } catch (err) {
      toast.error('Failed to load expense payments ledger')
      console.error(err)
    } finally {
      setLoading(false)
    }
  ...
  const filteredPayments = payments.filter((p) => {
    ...
    const matchesSearch = (
      (p.expense_title && p.expense_title.toLowerCase().includes(term)) ||
      (p.expense_category && p.expense_category.toLowerCase().includes(term)) ||
      ...
      String(p.expense_id).includes(term)
    )
  ...
  const totalAmountPaid = filteredPayments.reduce((sum, p) => sum + (parseFloat(p.amount) || 0), 0)
      ...
      <div className="w-full flex flex-col gap-6">
        ...
        <div className="bg-white border border-slate-200 shadow-sm p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
              ...
              Expense Payments Ledger
            </h1>
            <p className="text-xs text-slate-500 mt-0.5">Comprehensive record of operational expense payments, payment methods, and transaction notes.</p>
          </div>
          <Link
          ...
          >
            <BiReceipt className="text-base" /> Expense Console
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div className="bg-white border border-slate-200 shadow-sm p-6 flex flex-col justify-between hover:shadow-md transition">
            ...
            <div>
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Total Expense Paid</span>
          ...
          <div className="bg-white border border-slate-200 shadow-sm p-6 flex flex-col justify-between hover:shadow-md transition">
            ...
            <div>
              ...
              <p className="text-slate-500 text-xs mt-1">Direct cash payments</p>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 shadow-sm p-6 flex flex-col justify-between hover:shadow-md transition">
            ...
            <div>
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Bank & Digital Payments</span>
              <h3 className="text-2xl font-extrabold text-slate-800 mt-1">{formatCurrency(digitalPaidTotal)}</h3>
              <p className="text-slate-500 text-xs mt-1">Bank, MFS & Cards</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex-1 w-full relative">
            ...
            <input
              type="text"
              placeholder="Search by title, category, payment ID, branch or note..."
              value={search}
        ...
        {loading ? (
          <div className="w-full h-64 bg-white border border-slate-200 shadow-sm flex items-center justify-center text-slate-500 gap-2">
            ...
            <span className="text-xs font-medium">Loading expense payment records...</span>
          </div>
              ...
              <table className="w-full border-collapse text-left text-xs text-slate-600">
                <thead className="bg-slate-100/70 text-xs font-bold text-slate-700 uppercase tracking-wider border-b border-slate-200">
                  <tr>
                    ...
                    <th scope="col" className="px-4 py-3">Expense Title</th>
                    <th scope="col" className="px-4 py-3">Category</th>
                    <th scope="col" className="px-4 py-3">Branch</th>
                    <th scope="col" className="px-4 py-3">Method & Note</th>
                ...
                <tbody className="divide-y divide-slate-100 border-t border-slate-100 font-medium">
                  {filteredPayments.map((pm) => (
                    <tr key={pm.payment_id} className="border-b border-slate-200 text-xs text-slate-700 hover:bg-slate-50 transition">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col">
                          <span className="font-bold text-slate-800 text-xs flex items-center gap-1 capitalize">
                            <BiCreditCard className="text-slate-400" />
                            {(pm.payment_method || 'cash').replace('_', ' ')}
                          </span>
                          {pm.note && (
                            <span className="text-slate-400 text-[11px] italic mt-0.5">
                              {pm.note}
                            </span>
                          )}
                        </div>
          ...
          <div className="w-full py-16 bg-white border border-slate-200 shadow-sm flex flex-col items-center justify-center text-slate-400 gap-2">
            ...
            <p className="font-bold text-slate-600 text-sm">No expense payment records found</p>
