...
export async function GET(req) {
  try {
    ...
    const [
      ...
      query("SELECT COALESCE(SUM(stock), 0)::int AS count FROM stocks"),
      query("SELECT COALESCE(SUM(s.stock * v.sale_price), 0)::float AS val, COALESCE(SUM(s.stock * v.purchase_price), 0)::float AS cost FROM stocks s JOIN product_variants v ON s.variant_id = v.variant_id"),
